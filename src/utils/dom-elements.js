//form elements
export const sectionAppointmentAdd = document.querySelector('#appointment-add');
export const appointmentForm = document.querySelector('#appointment-form');

export const pet = document.querySelector('#pet');
export const tutor = document.querySelector('#tutor');
export const telephone = document.querySelector('#telephone');
export const service = document.querySelector('#service');
export const date = document.querySelector('#date');
export const time = document.querySelector('#time');
export const submitButton = document.querySelector('.form-submit');

//appointments list
export const selectDateInput = document.getElementById("selected-date");
export const newAppointmentButton = document.querySelector('.new-appointment-button');

//others
export const blurFilter = document.querySelector('.blurFilter');

//confirm card
export const confirmCard = document.querySelector('.confirm-card');
export const confirmText = confirmCard.querySelector('h2');
export const confirmButton = confirmCard.querySelector('.confirm-button');
export const noButton = confirmCard.querySelector('.no-button');